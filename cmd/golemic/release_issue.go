...
func runReleaseIssue(args []string, stdout, stderr io.Writer, getenv func(string) string, executor preflight.Executor) int { //nolint:cyclop,funlen,gocognit
	fs := flag.NewFlagSet("release-issue", flag.ContinueOnError)
	fs.SetOutput(stderr)

	var numberFlag int
	var reasonFlag string
	fs.IntVar(&numberFlag, "number", 0, "Issue number to release (required, positive integer)")
	fs.StringVar(&reasonFlag, "reason", "", "Release reason: done|failed|abandoned (required)")
	...
	// BR-004: validate --reason before any gh call.
	switch reasonFlag {
	...
	result, releaseErr := claim.Release(executor, numberFlag, ghUser.Login, devToken, reasonFlag)
