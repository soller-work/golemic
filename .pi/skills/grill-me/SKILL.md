...
1. **Inspect first.** Read the relevant code: modules, routes, conventions, test/verify commands. Classify the slice yourself using `references/slice-types.md` and set `change_type` yourself:
   ...
   **E2E obligation (feature slices):** Fill `e2e.scenarios` with concrete scenario names (lowercase snake_case) plus descriptions, or obtain the user's explicit waiver (`e2e.waiver`, `granted=true`, with their documented reason). If the slice requires modifying existing E2E tests, name the affected files to the user and set `e2e_modify_approved=true` only with their explicit approval — never infer it.
...
4. **Fill the slice.** Use `slice.py write` (or `new` + `set`) to populate every field from intent + code + answers. Record settled decisions and approved assumptions in `business_rules` (and `scope` for constraints); reserve `blockers` for open questions and genuine blockers that must stop autonomous implementation. The three `kind` values all force `blocked`: `question` — an open product decision not yet answered; `blocker` — an external dependency or hard blocker; `assumption` — an unconfirmed guess you want a human to veto before implementation proceeds. Do not use `blockers` to preserve a record of decisions already made; those belong in `business_rules` or `scope`.
...
Finalize sets `readiness: ready` when `blockers` is empty and validation passes; any non-empty `blockers` list — regardless of `kind` — yields `blocked`. There is no `resolved` state: an entry in `blockers` always blocks. Decisions that have been made belong in `business_rules` or `scope`, not in `blockers`. If evidence is genuinely unavailable, use `slice.py finalize <path> --blocked` and describe what's missing. Never mark `ready` just to stop.
