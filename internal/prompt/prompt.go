...
5. For each finding that is anchored to a specific file and line, call:
   ` + "`" + `golemic review-comment --pr {{.PRNumber}} --path <file> --line <line> --body "<finding>"` + "`" + `
   **1-retry contract:** if ` + "`" + `review-comment` + "`" + ` exits 2 (` + "`" + `ANCHOR_FAILED` + "`" + `), retry once with corrected coordinates.
   If the second attempt also exits 2, do **not** retry further — fold the finding into the submit-review body.
   Any finding for which you cannot determine a precise (file, line) anchor must also go into the body.
6. Call exactly one: ` + "`" + `golemic submit-review --verdict approved|changes_requested --body "..." --pr {{.PRNumber}} --merge-confidence high|low` + "`" + `
   The body **must** summarise all findings, including any that could not be pinned as inline comments.
`
