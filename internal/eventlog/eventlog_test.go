...
// AC-008: ValidateReviewSubmittedPayload enforces new fields (BR-006)
...
func TestValidateReviewSubmitted_NegativeInlineCommentCount_AC008(t *testing.T) {
	...
	if err := ValidateReviewSubmittedPayload(raw); err == nil {
		t.Error("expected error for negative inlineCommentCount, got nil")
	}
}
...
func TestValidateReviewSubmitted_ZeroInlineCommentCount_Valid_AC008(t *testing.T) {
	...
	if err := ValidateReviewSubmittedPayload(raw); err != nil {
		t.Errorf("expected no error for inlineCommentCount=0, got: %v", err)
	}
}
...
func TestValidateAutomergeConflictRetryPayload_Empty(t *testing.T) {
	if err := ValidateAutomergeConflictRetryPayload(nil); err == nil {
		t.Error("expected error for nil payload, got nil")
	}
}

func TestWriteAutomergeConflictRetry_RejectsInvalidPayload(t *testing.T) {
	dir := t.TempDir()
	path := filepath.Join(dir, "events.jsonl")
	w, err := NewWriter(path)
	if err != nil {
...
func TestWriteAutomergeConflictRetry_RoundTrip(t *testing.T) {
	dir := t.TempDir()
	path := filepath.Join(dir, "events.jsonl")
	w, err := NewWriter(path)
	if err != nil {
	...
	if events[0].Type != EventAutomergeConflictRetry {
		t.Errorf("type: got %q, want %q", events[0].Type, EventAutomergeConflictRetry)
	}
}
