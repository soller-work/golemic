...
type Config struct {
	...
	MaxConflictResolutionAttempts int                  `json:"max_conflict_resolution_attempts,omitempty"`
...
type configRaw struct {
	...
	MaxConflictResolutionAttempts *int               `json:"max_conflict_resolution_attempts"`
...
func Load(repoRoot string) (*Config, error) {
	...
	// Extract max_conflict_resolution_attempts (optional; default 2)
	if raw.MaxConflictResolutionAttempts != nil {
		config.MaxConflictResolutionAttempts = *raw.MaxConflictResolutionAttempts
		if config.MaxConflictResolutionAttempts <= 0 {
			return nil, fmt.Errorf("field 'max_conflict_resolution_attempts' must be > 0, got %d in config file %s",
				config.MaxConflictResolutionAttempts, configPath)
		}
	} else {
		config.MaxConflictResolutionAttempts = 2
	}
