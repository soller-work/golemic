...
func (l *Loop) claim(issueNum int, env map[string]string) bool {
	...
	if errors.As(err, &ee) {
		switch ee.ExitCode {
		case 3:
			fmt.Fprintf(l.stderr, "run-loop: race lost on issue #%d\n", issueNum) //nolint:errcheck
		case 4:
			fmt.Fprintf(l.stderr, "run-loop: issue #%d not takeable\n", issueNum) //nolint:errcheck
		default:
			fmt.Fprintf(l.stderr, "run-loop: claim-issue #%d failed: exit %d\n", issueNum, ee.ExitCode) //nolint:errcheck
		}
	} else {
		fmt.Fprintf(l.stderr, "run-loop: claim-issue #%d failed: %v\n", issueNum, err) //nolint:errcheck
	}
	return false
}
