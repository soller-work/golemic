...
After completing the review, you must emit **exactly one** `gm_review_submit` with `{ verdict, confidence, body }`. The value is the sole switch that governs whether the runner auto-merges the PR:
...
10. Reuse ladder and SHORTCUT markers: confirm the reuse ladder was respected — no re-implemented helper, stdlib, or platform capability, no unrequested abstraction or dependency. Confirm that any deliberate corner-cut carries a `// SHORTCUT:` marker naming the ceiling and upgrade path. A silently over-built or silently cut path is graded on the existing severity scale (P1 for correctness/security impact, P2 for spec deviations and missing guards, P3 for purely cosmetic reuse preferences).
