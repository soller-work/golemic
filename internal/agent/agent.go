...
func RunRole(ctx context.Context, cfg RoleConfig) (exitCode int, paths TranscriptPaths, err error) {
	...
	// Both dev and reviewer sessions are scoped per round so each new round starts a fresh
	// pi session with no prior-round transcript replay, while attempts within the same round
	// still resume (issue-212, issue-219).
